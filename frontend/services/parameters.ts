...
export const parametersApi = {
  async search(params: SearchParams = {}): Promise<SearchResults> {
    ...
    if (params.excludeRequirements?.length) {
      searchParams.append('excludeRequirements', params.excludeRequirements.join(','));
    }
    if (params.excludeObjectives?.length) {
      searchParams.append('excludeObjectives', params.excludeObjectives.join(','));
    }
    if (params.excludeConstraints?.length) {
      searchParams.append('excludeConstraints', params.excludeConstraints.join(','));
    }
