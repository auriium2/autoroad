...
class Marker(BaseModel):
    courseId: str = Field(..., min_length=1, max_length=50, pattern=r"^[a-zA-Z0-9.\-_:]+$", description="Course subject ID (e.g., '6.1200', '18.01')")
...
class ObjectiveConfig(BaseModel):
    key: str = Field(..., min_length=1, max_length=100, pattern=r"^[a-z0-9_]+$", description="Objective key (e.g., 'minimize_units')")
    parameters: dict[str, object] = Field(default_factory=dict, description="Optional parameters for the objective")


class ConstraintConfig(BaseModel):
    key: str = Field(..., min_length=1, max_length=100, pattern=r"^[a-z0-9_]+$", description="Constraint key (e.g., 'ban_prefix')")
...
class OptimizationRequest(BaseModel):
    ...
    planningYear: str | None = Field(default=None, pattern=r"^\d{4}-\d{4}$", description="Planning year (e.g., '2024-2025')")
