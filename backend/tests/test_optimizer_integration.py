...
- Solo courses not requiring prerequisites
...
class TestOptimizerIntegration:
    ...
    def test_solo_course_ignores_prerequisites(self):
        """
        Regression test: Solo courses should not require prerequisites.
        
        Scenario: 18.02 marked as solo should not need 18.01.
        """
        courses_df = create_simple_courses_df()
        model = cp_model.CpModel()
        
        markers = [
            Marker(course_id='18.02', status='solo', section=0),  # Freshman Fall, solo
        ]
        ...
        # Pass solo courses to skip prerequisite checking
        solo_course_ids = {'18.02'}
        add_prerequisite_constraints(model, take_vars, courses_df, 2024, prereq_trees, solo_course_ids)
        
        # Should be feasible even without 18.01
        solver = cp_model.CpSolver()
        status = solver.Solve(model)
        assert status in [cp_model.OPTIMAL, cp_model.FEASIBLE], "Solo course should ignore prerequisites"
